...
def besoinEnMission(m,l_m, l_a, nbmiss, affect,nb,t,i):
    ...
    for a in l_a:
        ...
        #Est-ce que l'avion rempli les critères
        if (m.opex == i and m.type_avion == a.type_avion and capaciteMission(a,m)
            ...
            if t<=2:
                ...
                break
            h=0
            #test si l'avion est déjà affecté en mission sur les deux derniers mois
            for k in l_m:
                if (affect(t-1)[a]==k.nom or affect(t-2)[a]==k.nom):
                    h=1
                    #si oui, on le rend indispo, on n'enchaine pas les missions
            if h==0:
                liste.append(a)
    return liste
...
def affectMaint(a, t, df, listeMaintenance, binary): #la variable binaire correspond au fait de bloquer la maintenance
    if t > 1:
            ...
            for i in range(0,len(l_maint)):
                if a.proch_maint == l_maint[i].nom:
                    for j in range(0, int(l_maint[i].duree)):
                        df.xs(t + j)[a] = (l_maint[i].nom+"$"+str(binary))
                    a.pot_mois = l_maint[i].gain_mois
                    a.pot_horaire = l_maint[i].gain_heures
                    a.proch_maint = l_maint[(i+1)%len(l_maint)].nom
                    break
